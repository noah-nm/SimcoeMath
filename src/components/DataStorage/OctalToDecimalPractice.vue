    ...
    <v-container>
      <v-row justify="center">
        <v-col cols="12" md="5">
          ...
          <v-row class="mt-8">
            <v-col>
              <h3 class="text-center">
                Binary Number: <strong class="text-blue">{{ octalNumber }}</strong>
              </h3>
            </v-col>
          </v-row>
          ...
          <v-row>
            <v-col>
              <h2 class="text-center">The Correct Answer is: <strong v-if="reveal">{{ octalDecimal }}</strong></h2>
            </v-col>
          </v-row>
        </v-col>
      </v-row>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      function generateBinary() {
        const randomOctal = Array.from({ length: 5 }, () => Math.floor(Math.random() * 8)).join('');
