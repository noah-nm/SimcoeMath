<template>
    <v-container>
      <v-row justify="center">
        <v-col cols="12" md="5">
          <h1 class="text-center">Find the result of the binary multiplication</h1>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      function performAddition() {
        const randomBinary1 = Array.from({ length: 4 }, () => Math.round(Math.random())).join('');
        const randomBinary2 = Array.from({ length: 4 }, () => Math.round(Math.random())).join('');
