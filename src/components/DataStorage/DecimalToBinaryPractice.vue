<template>
    <v-container>
      <v-row justify="center">
        <v-col cols="12" md="5">
          <h1 class="text-center">Convert the decimal number to binary</h1>
          ...
          <v-row justify="center">
            <v-col class="d-flex" cols="auto">
              <v-btn class="mr-6 pr-6 pl-6"@click="generateDecimal" color="primary">New Number</v-btn>
          ...
          <v-row>
            <v-col>
              <h2 class="text-center">The Correct Answer is: <strong v-if="reveal">{{ decimalBinary }}</strong></h2>
            </v-col>
          </v-row>
        </v-col>
      </v-row>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      function generateDecimal() {
        const bitLength = Math.floor(Math.random() * 8) + 1;
