        ...
        <v-col cols="12" md="5">
          ...
          <v-row class="mt-8">
            <v-col>
              <h3 class="text-center">
                Hexadecimal Number: <strong class="text-blue">{{ hexadecimalNumber }}</strong>
              </h3>
            </v-col>
          </v-row>
          ...
          <v-row justify="center">
            <v-col class="d-flex" cols="auto">
              <v-btn class="mr-6 pr-6 pl-6"@click="generateHex" color="primary">New Number</v-btn>
          ...
          <v-row>
            <v-col>
              <h2 class="text-center">The Correct Answer is: <strong v-if="reveal">{{ hexadecimalDecimal }}</strong></h2>
            </v-col>
          </v-row>
        </v-col>
      </v-row>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      function generateHex() {
        const randomHex = Array.from({ length: 5 }, () => Math.floor(Math.random() * 16).toString(16)).join('').toUpperCase();
