  ...
  export default defineComponent({
    ...
    setup() {
      ...
      function generateDecimal() {
        const randomDecimal = Math.floor(Math.random() * 1000); // Generate a random decimal number between 0 and 999
        decimalNumber.value = randomDecimal;
        userInput.value = '';
        decimalOctal.value = randomDecimal.toString(8); // Convert decimal to octal string
        reveal.value = false;
        result.value = null;
      }
